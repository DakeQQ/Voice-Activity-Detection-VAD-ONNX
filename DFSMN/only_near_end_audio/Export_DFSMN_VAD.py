...
INPUT_AUDIO_LENGTH = 31841              # Maximum input audio length: the length of the audio input signal (in samples) is recommended to be greater than 31841. Higher values yield better quality. It is better to set an integer multiple of the NFFT value.
...
class DFSMN_VAD(torch.nn.Module):
    def __init__(self, dfsmn_vad, iccrn, alpha_predictor, custom_stft_A, custom_stft_A2, custom_stft_B, nfft_A, nfft_B, k, max_len, pre_emphasis, sample_rate, n_mels):
        ...
        self.pow_far_white_noise = torch.randn((1, 1, n_mels + n_mels, max_len, k), dtype=torch.float16).pow(2)
        self.far_comp_white_noise = torch.randn((1, 1, 2, n_mels + n_mels, max_len), dtype=torch.float16)
    ...
    def forward(self, audio):
        near_end_audio = audio.float() * self.inv_int16
        ...
        stft_len = near_real_part_B.shape[-1].unsqueeze(0)
        ...
        pow_far = self.pow_far_white_noise[:, :, :, :stft_len].float()
        ...
        far_comp = self.far_comp_white_noise[..., :stft_len].float() * torch.abs(alpha)
